# Spike 003: stateful restart-based `model_a` backend
...
per slave model (`--model model_a|model_b`; well names, initial pressure,
...
|---|---|---|---|
| 2024 | `MODEL_A_2024.DATA` (base) | `EQUIL` | — |
| 2025 | `MODEL_A_2025.DATA` (continuation) | `RESTART 'MODEL_A_2024' 1 /` | `MODEL_A_2024.UNRST` + `.EGRID` staged beside the deck |
| 2026 | `MODEL_A_2026.DATA` (continuation) | `RESTART 'MODEL_A_2025' 2 /` | `MODEL_A_2025.UNRST` + `.EGRID` |

Year-end `WOPR`, `WWPR`, `WBHP`, `FPR`, `FOPT`, `FWPT` are extracted with
OPM's `summary -r` CLI and emitted as `slave_rates_model_a.csv` rows plus a
...
1. **Restart is a plain deck keyword.** `RESTART 'CASE' STEP /` in SOLUTION
   ...
   and groups from the restart file (`Adding well A-P1 from restart file`).
...
5. **The Spike-001 toy grid depletes to BHP within weeks** (FPR 350 → 301 bar
   in one year; the 321.9-bar A-P2 constraint then exceeds reservoir pressure
...
python3 spikes/003-opm-model-n-restart/opm_model_n_restart_adapter.py \
  --constraints output/demo/realization-0/coupling/network_constraints_model_a.csv \
  --output-dir output/opm-model-n-restart-spike \
  --model model_a
```

`--model model_b` runs the same chain for the hdn slave (B-P1/B-P2, 315 bar
...
```text
<output-dir>/
├── slave_rates_model_a.csv      # 3 years x 2 wells, exchange schema
├── restart_report.json          # constraints, per-year results, checks
└── year-2024|2025|2026/
    ├── MODEL_A_<year>.DATA      # rendered base or continuation deck
    ├── MODEL_A_<prev>.UNRST/.EGRID  # staged restart artifacts (cont. years)
...
(`output/demo/realization-0/coupling/network_constraints_model_a.csv`),
Flow 2025.10:

| Year | FOPT sm³ | FWPT sm³ | FPR bar | A-P1 q_liq sm³/d (req BHP) | A-P2 q_liq sm³/d (req BHP) |
|---|---:|---:|---:|---:|---:|
...
- Restart files are staged by copying `.UNRST`/`.EGRID`; a field implementation
  should preserve the same state/provenance contract under its FMU runpath.
- Coupling exchange occurs at configured report periods, not at Flow nonlinear
  ministeps.

## Current integration

The adapter is fully wired into `ert/bin/scripts/run_coupled.py` for both
`model_a` and `model_b`. The primary `coupling.json` uses these two Flow
restart slaves plus the Spike 004 Flow NETWORK master, with no prescribed
profiles. Realization-specific productivity draws scale permeability in the
actual rendered base and continuation decks. The driver under-relaxes the raw
Flow responses and checks convergence on the unrelaxed fixed-point target.

The final selected ERT run (`0-2`) converged in 12-13 iterations per member,
and every reservoir/master PRT had `Warnings 0`, `Errors 0`, and `Problems 0`.
See the main README and
`docs/GUIA_IMPLEMENTACION_ACOPLAMIENTO_BIDIRECCIONAL.md` for the active
