...
- Table 1: model_a wells (A-P1/A-P2). BHP ~322-380 bar depending on THP,
  mildly declining with rate; used by the model_a slave wells.
- Table 2: model_b wells (B-P1/B-P2). Identical to table 1: with the
...
- Table 3: trunk MANIFOLD->FIELD. Values ARE the upstream node pressure
  ...
  BHP caps (350 model_a / 315 model_b).
...
def trunk_table(
    ...
    pressure + wellbore hydrostatic below the slave BHP caps (350 model_a
    / 315 model_b).
    """
    wct_axis = [0.0, 0.5]
    gor_axis = [0.0, 300.0]
...
def generate() -> str:
    parts = [
        ...
        "-- Table 1: model_a wells. Table 2: model_b wells. Table 3: trunk.",
